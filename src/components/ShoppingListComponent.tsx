...
const ShoppingListComponent: React.FC<Props> = ({ list, onListUpdate, onListDelete, isOnline, showToast }) => {
  ...
  return (
    <div>
      <div className="flex justify-between items-center gap-2 mb-4 border-b pb-2">
        <div className="flex-grow min-w-0">
          {isEditing ? (
            ...
            <div
              className="flex items-center gap-2 cursor-pointer group p-1 -ml-1 rounded-md"
          ...
          <span className="text-xs sm:text-sm text-gray-500 ml-1">{formatDate(list.createdAt)}</span>
        </div>
      ...
      <form onSubmit={handleAddItem} className="flex flex-col sm:flex-row gap-2 mb-4">
        ...
        <input
          ...
          placeholder="Qtde"
          className="w-full sm:w-20 p-2 border border-gray-300 rounded-md"
        />
        <input
          type="number"
          value={newItemPrice}
